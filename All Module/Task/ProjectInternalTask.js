...
const TaskPicker = ({
  ...
  const grouped = useMemo(() => {
    ...
    return [...sectionKeys, ...noneKey].map((k) => ({
      key: k,
      label: k === "__none__" ? "Không có Nhóm" : k,
      tasks: map[k],
    }));
  }, [filtered, allTasks]);
...
const TimesheetModal = ({
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      projectInternalId: extractId(PROJECT_ID),
    };
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          lbl("🗂 Nhóm công việc (Section)"),
          React.createElement("input", {
            placeholder: "Chọn hoặc nhập tên nhóm...",
...
const ProjectInternalTasksTab = () => {
  ...
  const handleStatus = useCallback(
    async (id, newSt, type) => {
      ...
      try {
        ...
        message.error("Lỗi Backend: Không thể cập nhật trạng thái");
        reload();
      }
    },
  ...
  const handleDeleteTask = useCallback(
    async (id, type, title) => {
      const isSub = type === "subTask";
      const label = isSub ? "công việc phụ" : "công việc chính";
      Modal.confirm({
        title: `Xác nhận xoá ${label}`,
        content: `Bạn có chắc muốn xoá "${title}" không? Hành động này không thể hoàn tác!`,
        okText: "Xoá",
        okType: "danger",
        cancelText: "Huỷ",
        onOk: async () => {
