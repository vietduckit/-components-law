...
**Post-plan addition:** `pgsql/deploy-contract-payment-status.sh` — a one-command deploy script wrapping both halves of Task 7 (the SQL migration and the Nocobase field registration) via `psql` + `curl`, added after plan completion so deploying to a different environment doesn't require pasting `JsField/RegisterContractPaymentStatusFields.js` into a browser console by hand. Both its skip-path (fields already registered) and create-path (field metadata deleted, script recreates it) were run against the local instance and verified, not just read as correct — which is how a real bug was caught: a bash `mktemp -d` temp directory (a POSIX-style path under Git Bash/MSYS) doesn't reliably resolve when handed to `node.exe` as a file path argument, so the script's temp files live under its own resolved script directory instead.
