...
**Verify during implementation** (flagging honestly rather than assuming): confirm NocoBase Workflow's Loop node can iterate a JS-evaluated array parsed out of a JSON column (`paymentSchedule.installments`) rather than only over a Query-record result set. The `workflows.md` reference doc in this repo only documents "Loop | Lặp qua array records" at a high level — this needs a hands-on check the first time WF1 is built, with a Calculation node producing the array if the Loop node needs a literal array input.
