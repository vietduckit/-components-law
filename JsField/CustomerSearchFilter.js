 ...
 * Deployment: Customer module (collection "customers").
 *
 * This is JsField/GenericSearchFilter.js with CONFIG filled in for the
 * Customer module. Field names below were confirmed directly against the
 * live Nocobase "Customer - Configure fields" admin UI (screenshots).
 *
 * STILL NEEDED before this is deployable — see the 3 TODOs below:
 *   1. `targetBlockUid` is empty — get it from the customers table block's
 *      designer menu ("Copy block UID") and fill it in.
 *   2. Four `status`-type filters (`status`, `customerType`, `gender`,
 *      `source`) have their real field/type confirmed (all "Single select"
 *      strings) but their `options` (value/label pairs) were NOT visible in
 *      the field-list screenshot — only "Single sel..." (truncated), no
 *      enum shown. Do NOT assume `source` matches `leads.source`'s enum
 *      (googleAds/facebookAds/...) — it's a different collection's field
 *      and may differ. Open each field's "Edit field" dialog (same way you
 *      did for Lead's `status`/`source`) and share the Option value/label
 *      list; `options: []` below is a placeholder, not a real empty enum.
 *   3. `assignees` (belongsTo, "Many to one", field name `assignees`) has
 *      no separate flat FK column in the field list, matching the same
 *      shape as Lead's `Assignees` and Quotation's `lawyers`/
 *      `approvalLawyers` — so `relationKey: 'id'` is applied by analogy.
 *      Its target collection is ASSUMED to be `lawyers` (same as those
 *      other Assignees-style fields elsewhere in this app) — this was NOT
 *      independently confirmed for `customers.assignees` specifically.
 *      Verify (or correct) once deployed: if the relation dropdown comes
 *      back empty/errors, check the field's real target collection.
 *
 * Field notes (confirmed):
 *   - `internalCompanyId` is a flat scalar FK column (separate from the
 *     `internalCompany` belongsTo relation also present) — filtered
 *     directly, same pattern as Lead/Quotation.
 *   - Search fields (`customerName`, `email`, `phone`, `shortName`) are all
 *     confirmed real fields on `customers` (unlike the Quotation deployment
 *     history, these were checked against the actual field list before use,
 *     not copy-pasted from another module).
 *
 * HOW TO USE THIS FILE:
 * 1. Fill in the 3 TODOs above.
 * 2. Copy this entire file into the Customer module's JS Field/Action block
 *    in Nocobase.
...
const CONFIG = {
  ...
  filters: [
    ...
    },
    {
      ...
      options: [
        // TODO: fill in real Option value/label pairs — see file header note 2
        // (do NOT assume this matches leads.source's enum)
      ],
      showCounts: true,
    },
    {
