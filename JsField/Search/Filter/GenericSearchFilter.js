...
const CONFIG = {
  ...
  //       excludeValues: [],
  //       sort: 'createdAt',
  //       // Optional: only list records the target table actually uses
  //       // through a flat FK column (e.g. customers that have >= 1 case),
  //       // limited to the currentUserScope when that is enabled.
  //       // onlyUsedBy: { field: 'customerId', respectCurrentUserScope: true },
  //       // Optional: append "(other label)" to entries sharing a label (+ "#id"
  //       // only if that other label is shared too).
