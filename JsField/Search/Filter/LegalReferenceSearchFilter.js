 ...
 * Deployment: Legal Reference module ("Case Reference" in the UI, collection
 * "legalReference").
 *
 * This is JsField/Search/Filter/GenericSearchFilter.js with CONFIG filled in
 * for the Legal Reference module, following the same structure as
 * JsField/Search/Filter/ContractSearchFilter.js / QuotationSearchFilter.js.
 *
 ...
 *
 * Field notes — confirmed two ways: (a) the live "Case Reference - Configure
 * fields" admin UI (3 screenshots covering the full field list: id,
 * internalCompanyId, managerId, createdAt, createdBy, updatedAt, updatedBy,
 * title, referenceCode, internalCompany, documents, folders, notes,
 * description, status, priority, referenceKind, sourceSpace, sourceType,
 * sourceDocumentIds, sourceFolderIds, sourceTitle, sourceSummary, linkMode,
 * legalStudy, cases, manager, members), and (b) the actual create payload in
 * All Module/Document/CaseReferenceCreateBlock.js:792-802:
 *   - `collection name`: LEGAL_REFERENCE_RESOURCES (line 22-26) tries
 *     "legalReference" first — used as CONFIG.tableName.
 *   - `status` options: Active / Closed / Pending — confirmed EXACTLY as
 *     capitalized at CaseReferenceCreateBlock.js:1007-1011 (Select options
 *     for the `status` field). Note the capitalization is different from
 *     every other module's camelCase status values (toDo/inProgress/...) —
 *     kept verbatim, not normalized, since it's what the form actually sends.
 *   - `priority` options: low / medium / high — CaseReferenceCreateBlock.js:990-994.
 *   - `internalCompanyId` (label "Internal Company") — flat scalar FK,
 *     payload line 795, same shape as every other module.
 *   - `managerId` (label "Manager") is a flat scalar FK, but — unlike
 *     managerId/lawyerId/assignees fields in Case/Quotation/Contract, which
 *     target the separate `lawyers` collection — this one targets `users`
 *     DIRECTLY: its options come from `USER_RESOURCES = ["users"]`
 *     (line 36, fetched line 728, `userOptions` built at line 859-862 from
 *     that `users` list). Confirmed at payload line 799
 *     (`manager: { id: managerId }, managerId`). Because it targets `users`
 *     (not `lawyers`), it is safe to use directly in `currentUserScope.userFields`
 *     (no `lawyers.userId` hop needed) — same reasoning as Case's
 *     `projectManagerId` which also targets `users` directly.
 *   - `members` (label "Members") is a `belongsToMany` with no flat FK
 *     column, confirmed at payload line 800 (`members: memberIds.map(id => ({id}))`)
 *     — same `userOptions`/`users` target as `managerId`. Needs
 *     `relationKey: 'id'` for the filter control, and for
 *     `currentUserScope.relationFields` needs `targetKey: 'id'` (NOT
 *     `'userId'` — because `members` points straight at `users.id`, there is
 *     no intermediate `lawyers` record to hop through, unlike Case's
 *     `assignees` -> `lawyers.userId`).
 *   - `cases` (belongsToMany to `projects`) — confirmed at payload line 789/796
 *     (`caseIds` -> `cases: caseIds`, plain array of project ids). Included
 *     as a relation filter with `relationKey: 'id'`.
 *   - The `search` filter uses `title` (label "Reference Name"),
 *     `referenceCode`, and `description` (label "Reference Summary") — all
 *     three confirmed real columns from the screenshots and the create
 *     payload (`title`, `description` at payload lines 793-794;
 *     `referenceCode` confirmed field-list-only, a `sequence` type, not
 *     written by this create form but still a valid $iLike search target).
 *   - NOT included as filters (exist on the collection per the screenshots,
 *     but their values are never set by CaseReferenceCreateBlock.js — they
 *     appear to be populated by a separate linking/import flow, most likely
 *     JsField/JsLegalReferenceLinks.js, and their real option values were
 *     NOT independently confirmed for the `legalReference.linkMode`/
 *     `referenceKind` fields specifically — do not assume the "case"/
 *     "legal_study" values seen in JsLegalReferenceLinks.js's *local modal
 *     UI state* apply 1:1 to these collection fields without checking):
 *     `referenceKind`, `sourceSpace`, `sourceType`, `sourceDocumentIds`,
 *     `sourceFolderIds`, `sourceTitle`, `sourceSummary`, `linkMode`,
 *     `legalStudy`. Add them later the same way once confirmed.
 *
 * currentUserScope is ENABLED (per explicit request: "đảm bảo đáp ứng được
 * cho currentUserScope"), configured for a "My References" style scope even
 * though no native Data scope screenshot was provided for this module (unlike
 * Case/Contract) — Meet Any of: createdById = current user, managerId =
 * current user, members includes current user. If a real "My ..." block's
 * Data scope is set up later and differs from this, sync it the same way
 * ContractSearchFilter.js / CaseSearchFilter.js were synced to their screenshots.
 *
 * HOW TO USE THIS FILE:
 * 1. Fill in `targetBlockUid` below with the real block UID.
 * 2. Copy this entire file into the Legal Reference module's JS Field/Action
 *    block in Nocobase.
...
const CONFIG = {
  ...
  filters: [
    ...
    // },
    {
      ...
      key: 'search',
      label: 'Search',
      fields: ['title', 'referenceCode', 'description'],
      placeholder: 'Search by reference name, code, summary...',
    },
  ],

  // "My References" scope — Meet Any: createdById = current user,
  // managerId = current user, members includes current user.
  // `managerId` is a userFields entry (not relationFields) because it
  // targets `users` directly, same reasoning as Case's `projectManagerId`.
  // `members` needs relationFields with targetKey: 'id' (not 'userId') for
  // the same reason — it points straight at `users.id`, no `lawyers` hop.
  currentUserScope: {
    enable: true,
