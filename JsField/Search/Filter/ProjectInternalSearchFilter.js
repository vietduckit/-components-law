     ...
     * "My Works" tab — filters to projects where the current user is Manager,
     * a Member, or the creator. For the "All Works" tab, deploy a copy with
     * `currentUserScope.enable` set back to `false` (and a different
     * `targetBlockUid`) — everything else in CONFIG can stay identical.
     *
     ...
     *     BECAUSE of this, `projectManagerId` cannot go in `userFields` (would
     *     compare a lawyers.id column directly against the current users.id and
     *     silently hide every record the user manages — the original bug, found
     *     2026-07-29).
     *   - `projectManagerId` the column and `projectManager` the association are
     *     TWO SEPARATE fields in the Nocobase collection config (confirmed via
     *     screenshot of "Internal Work - Configure fields" / "Manager" field
     *     edit dialog, 2026-07-29): `projectManagerId` is a raw bigInt column;
     *     `projectManager` is the actual belongsTo field (foreign key
     *     `projectManagerId`, target collection Lawyer, target key = lawyers'
     *     id). Nested/relational filtering (e.g. for currentUserScope) MUST use
     *     the association name `projectManager`, not the column name
     *     `projectManagerId` — filtering `{ projectManagerId: { userId: {$eq} } }`
     *     500s live because that name only resolves to the plain scalar column,
     *     which has nothing to nest into. Scalar/exact-id filtering (the
     *     dropdown in `filters[]` below) can keep using `projectManagerId`
     *     directly — that shape doesn't need the association.
     *
    ...
    const CONFIG = {
      ...
      currentUserScope: {
        // Enabled here to reproduce the "My Works" tab's expected behavior:
        // show a project when the current user is its Manager OR a Member.
        // Deliberately NOT "Meet Any" like Case's "My Cases" (no createdById)
        // — confirmed against real data 2026-07-30: including createdById
        // made every project the logged-in user happened to create show up
        // regardless of Manager/Member, which is not what "My Works" means
        // here. If you deploy a separate "All Works" block from a copy of
        // this file, set `currentUserScope.enable` back to `false` there.
        enable: true,
        // Both target `lawyers`, compared via lawyers.userId. IMPORTANT: use the
        // association field name `projectManager`, NOT the raw FK column
        // `projectManagerId` — they're separate fields in the collection config
        // and nested filtering only resolves through the association name. See
        // file header note on projectManagerId.
        relationFields: [
          { field: 'projectManager', targetKey: 'userId' },
          { field: 'projectAssignees', targetKey: 'userId' },
        ],
